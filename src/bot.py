...
class Bot():
    ...
    def addClass(self, netid, password, className, discussion=None, lab=None, test=False):
        ...
        if (discussion != None and self.is_integer(discussion)):
            if (lab != None and self.is_integer(lab)):
                logging.warning('BOT @'+str(self.className)+': Trying to add discussion and lab')
                self.addDiscussion(discussion, only=False)
                self.addLab(lab, only=False)
            else:
                logging.warning('BOT @'+str(self.className)+': Trying to add discussion')
                self.addDiscussion(discussion, only=True)
        elif (lab != None and self.is_integer(lab)):
            logging.warning('BOT @'+str(self.className)+': Trying to add lab')
    ...
    def addDiscussion(self, discussion, only):
        ...
        try:
            ...
            logging.warning('BOT @'+str(self.className)+': Found discussion length and table-length ')
        except:
                # Over-estimate in case we cannot find the table length
                length = 20
                tablelength = 20
                logging.warning('BOT @'+str(self.className)+': Could not find discussion table-length. Using defaults')
        ...
        while(x < length and _found == False):
            ...
            for i in range(2, tablelength + 2):
                try:
                    ...
                    continue
            x += 5
        logging.warning('BOT @'+str(self.className)+': Successfully selected discussion')
    ...
    def addLab(self, lab, only):
        ...
        try: 
            if (only):
                ...
                tablelength = int(str(self.driver.find_element_by_xpath(
                    '//*[@id="SSR_CLS_TBL_R1$scroll$0"]/tbody/tr[3]/td/span[2]').text)[2:3])
            logging.warning('BOT @'+str(self.className)+': Found lab length and table-length ')
        except:
                # Over-estimate in case we cannot find the table length
                length = 20
                tablelength = 20
                logging.warning('BOT @'+str(self.className)+': Could not find lab table-length. Using defaults')
        ...
        while(x < length and _found == False):
            ...
            for i in range(2, tablelength + 2):
                try:
                    ...
                    continue
            x += 5
        logging.warning('BOT @'+str(self.className)+': Successfully selected lab')
    ...
    def getPreferences(self, password):
        # define settings from preference file, needs password to determine browser type
        if (int(Preferences.webdriver) == 0 or password == ''): # must use firefox is no password is given
    ...
    def logIntoStudentCenter(self, url, netid, password):
        ...
        logging.warning('BOT @'+str(self.className)+': Logged into Student Center')
    ...
    def selectSemester(self):
        ...
        try:
            ...
            logging.exception('BOT @'+str(self.className)+': Cannot find semester')
    ...
    def enterClassNbr(self):
        ...
        logging.warning('BOT @'+str(self.className)+': Entered class Nbr semester')
        src = self.driver.page_source
        if('The class number entered is a duplicate.  Try another'):
            logging.error('BOT @'+str(self.className)+': The class number eneted is a duplicate. Proceeding to compelte enrollment')
            self.driver.save_screenshot('bot_'+str(self.className)+'_error_screenshot_'+str(self._screentShotCount)+'.png')
    ...
    def clickNextToHome(self):
        ...
        while(not _home):
            ...
            if('your Shopping Cart and when you are satisfied' in src):
                _home = True
            if (count > 10):
                driver.save_screenshot('bot_'+str(self.className)+'_error_screenshot_'+str(self._screentShotCount)+'.png')
                self._screentShotCount += 1
                logging.error('BOT @'+str(self.className)+': Failed to return home after 10 clicks. Screenshot Saved')
    
    def completeEnrollment(self):
        with wait_for_page_load(self.driver):
            self.driver.find_element_by_id('DERIVED_REGFRM1_LINK_ADD_ENRL').click()
        src = self.driver.page_source
        if('Finish Enrolling to process your request for the classes listed below' not in src):
                self.driver.save_screenshot('bot_'+str(self.className)+'_error_screenshot_'+str(self._screentShotCount)+'.png')
                self._screentShotCount += 1
                logging.error('BOT @'+str(self.className)+': Failed to Proceed to Step 2 will try to remove classes and try again. Screenshot Saved')
        ...
        if('Error: Unable to complete your request' in src):
                self.driver.save_screenshot('bot_'+str(self.className)+'_error_screenshot_'+str(self._screentShotCount)+'.png')
                self._screentShotCount += 1
                logging.error('BOT @'+str(self.className)+': Failed to Finish Enrolling. Screenshot Saved')

        _time = time.time() - self._start
        logging.warning('BOT @'+str(self.className)+': Time to complete: '+str(_time))
    ...
    def waitForEnrollmentWindow(self, year):
        ...
        while(waiting):
            if (startDate < datetime.datetime.now()):
                ...
                loaded = False
                _time = time.time() # keep track of how long app took to get class
                while(not loaded):
                    src = self.driver.page_source
                    if('Select classes to add' in src):
                        loaded = True
                        logging.warning('BOT @'+str(self.className)+': Enrollment window is open')
                        break
                    with wait_for_page_load(self.driver):
                        self.driver.refresh()
                        logging.warning('BOT @'+str(self.className)+': Hard Refresh')
