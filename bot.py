...
import requests
import thread, time, os, datetime
...
class Bot(): 
    ...
    def waitForEnrollmentWindow(self, year):
        if(int(year) == 1 ): #freshman
            startDate = datetime.datetime.strptime("4/20/15/07/00/02", "%m/%d/%y/%H/%M/%S")
        if(int(year) == 2 ): #sophmore
            startDate = datetime.datetime.strptime("4/16/16/07/00/00", "%m/%d/%y/%H/%M/%S")
