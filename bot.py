...
class Bot(): 
	...
	def addClass(self, netid, password, className, discussion = None, lab = None, year = 1, test = False, hidden = False ):
		if (hidden == False):
	...
	def addLab(self, lab, only):
		...
		if (only ==True):
			...
			try:
				...
				tablelength = int(str(self.driver.find_element_by_xpath('//*[@id="SSR_CLS_TBL_R2$scroll$0"]/tbody/tr[3]/td/span[2]').text)[2:4])
			except:
				print "Can't find length"
				length = 20
				tablelength = 5
